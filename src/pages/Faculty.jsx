...
export default function Faculty() {
const [faculty, setFaculty] = useState([]); // no dummy
const [loading, setLoading] = useState(true);
const [err, setErr] = useState(null);
...
if (list.length === 0) {
setErr("No faculty members available.");
}
} catch (error) {
console.error("Faculty fetch error:", error);
if (mounted) {
setErr("Failed to load faculty data.");
setFaculty([]);
}
} finally {
if (mounted) setLoading(false);
...
<p className="col-span-full text-center text-slate-500">No faculty members available.</p>
) : (
faculty.map((f) => (
<div
key={f._id ?? f.id}
